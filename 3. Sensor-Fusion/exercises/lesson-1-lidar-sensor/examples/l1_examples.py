...
def vis_range_channel(frame, lidar_name, vis=True):
...
def print_vfov_lidar(frame, lidar_name):
    ...
    print("vfov angle = " + '{0:.2f}'.format(vfov_rad*180/np.pi) + "°")

    lidar_name = dataset_pb2.LaserName.TOP
    calib_lidar = [obj for obj in frame.context.laser_calibrations if obj.name == lidar_name][0]
    print(f"Number of individual laser LEDs of the top LiDAR sensor: {len(calib_lidar.beam_inclinations)}")
