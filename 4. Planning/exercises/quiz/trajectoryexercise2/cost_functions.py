...
def stays_on_road_cost(traj, target_vehicle, delta, T, predictions):
    """
    Penalizes trajectories where the vehicle leaves the road boundaries.
    """
    _, d_coeffs, t = traj
    d = to_equation(d_coeffs)

    road_left_boundary = 0
    road_right_boundary = ROAD_WIDTH
    cost = 0
    dt = float(T) / 100.0
    for i in range(100):
        t = dt * i
        d_pos = d(t)
        if d_pos < road_left_boundary or d_pos > road_right_boundary:
            cost += 1
    return logistic(cost / 100)  # Normalize cost to a range of 0 to 1

def exceeds_speed_limit_cost(traj, target_vehicle, delta, T, predictions):
    """
    Penalizes trajectories where the vehicle exceeds the speed limit.
    """
    s_coeffs, _, t = traj
