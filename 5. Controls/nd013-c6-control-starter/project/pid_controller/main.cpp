...
// std::size_t get_closest_point_idx(double point_x, double point_y, std::vector<double> points_x, std::vector<double> points_y) {
//   int idx = 0;
//   double min_dist = 1e6;
...
// Find closest point in vector that is ahead of the current position
std::size_t get_closest_point_idx(double current_x, double current_y, double current_yaw, const std::vector<double>& points_x, const std::vector<double>& points_y) {
    int idx = -1;
    double min_dist = std::numeric_limits<double>::max();

    for (int i = 0; i < points_x.size(); ++i) {
        double dx = points_x[i] - current_x;
        double dy = points_y[i] - current_y;

        // Check if the point is ahead
        double forward_dist = dx * cos(current_yaw) + dy * sin(current_yaw);
        // Computes forward_dist, which is the projection of the vector from the current position to the point on the direction vector defined by current_yaw. 
        // This is used to check if the point is ahead of the current position (i.e., forward_dist must be positive).
        if (forward_dist > 0) {
...
int main ()
{
  ...
  // Reference for initialization: https://knowledge.udacity.com/questions/939702

  PID pid_steer = PID();
  pid_steer.Init(0.14, 0.014, 0.02, 1.2, -1.2);
  // 0.14, 0.014, 0.02
  // (0.14, 0.012, 0.02
  // (0.15, 0.01, 0.02)
  // (0.15, 0.008, 0.025
  ...
  pid_throttle.Init(0.15, 0.0025, 0.02, 1.0, -1.0);
  // (0.15, 0.0025, 0.02)
  // (0.17, 0.0022, 0.02)
  // (0.17, 0.0018, 0.02
  // (0.18, 0.0012, 0.03)
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          std::size_t idx_closest_point = get_closest_point_idx(
            x_position,
            y_position,
            yaw,  // Make sure this is the current yaw of the vehicle
            x_points,  // Trajectory x points
            y_points   // Trajectory y points
          );
          ...
          // Calculate the difference in angle between the current yaw and the angle between the current position and the next waypoint.
          // This help reduce the trajectory error at each waypoint and correct it.
          ...
          // Calculate the difference in the current velocity and the desired velocity computed by the path planner at the next waypoint
