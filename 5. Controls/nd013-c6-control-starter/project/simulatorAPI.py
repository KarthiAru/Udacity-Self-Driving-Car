...
carla_egg_path = "/home/ka/carla/PythonAPI/carla/dist/carla-0.9.15-py3.11-linux-x86_64.egg" 
print(carla_egg_path)
...
# Open a file for logging at the start of your main or outside any class
log_file = open("waypoint_log.txt", "w")
log_file.write("Time, WP_X, WP_Y, WP_Z, WP_Yaw, WP_Pitch, WP_Roll, Velocity, Car_X, Car_Y, Car_Z, Car_Yaw, Car_Pitch, Car_Roll\n")
...
#             previous_index = index
        ...
        if (sim_time - last_move_time) > wait_time:
            ...
            if len(way_points) > 1:
                ...
                # Log the waypoint and car position
                wp_t = way_points[0]
                car_t = self.player.get_transform()
                log_file.write(f"{sim_time}, {wp_t.location.x}, {wp_t.location.y}, {wp_t.location.z}, {wp_t.rotation.yaw}, {wp_t.rotation.pitch}, {wp_t.rotation.roll}, {v_points[0]}, {car_t.location.x}, {car_t.location.y}, {car_t.location.z}, {car_t.rotation.yaw}, {car_t.rotation.pitch}, {car_t.rotation.roll}\n")
...
def SpawnNPC(client, world, args, offset_x, offset_y):
    ...
    random.shuffle(spawn_points)  # Shuffle spawn points to randomize selection
    
    # for spawn_point in spawn_points:
    #     actor_spawn = carla.Transform(spawn_point.location + carla.Location(x=offset_x, y=offset_y), spawn_point.rotation)
    #     # Try to spawn the actor
    #     future = client.apply_batch([SpawnActor(blueprint, actor_spawn)])
    #     result = future[0].get()
    #     if not result.error:
    #         print(f"NPC spawned successfully at X: {actor_spawn.location.x}, Y: {actor_spawn.location.y}")
    #         obst_x.append(actor_spawn.location.x)
    #         obst_y.append(actor_spawn.location.y)
    #         return SpawnActor(blueprint, actor_spawn)
    #     else:
    #         print(f"Failed to spawn NPC at X: {actor_spawn.location.x}, Y: {actor_spawn.location.y}, Error: {result.error}")
